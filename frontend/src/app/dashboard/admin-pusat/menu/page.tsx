...
import { FiBook, FiPlus, FiEdit2, FiTrash2, FiChevronDown, FiChevronUp, FiSave, FiX, FiUpload, FiDownload, FiActivity, FiRefreshCw } from "react-icons/fi";
...
export default function MasterMenuPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [m, pt] = await Promise.all([menuApi.getAll(), portionTypeApi.getAll()]);
      setMenus(m);
      setPortionTypes(pt);
    } catch (e) {
      console.error(e);
  ...
  const saveMenu = async () => {
    ...
    try {
      if (editingMenu) {
        const updated = await menuApi.update(editingMenu.id, menuForm);
        setMenus(prev => prev.map(m => m.id === editingMenu.id ? updated : m));
      } else {
        const created = await menuApi.create(menuForm);
  ...
  const deleteMenu = async (id: string) => {
    if (!confirm("Hapus template master menu ini?")) return;
    try {
  ...
  const downloadTemplate = () => {
    const ws = XLSX.utils.json_to_sheet([
      {
        "Nama Menu": "Daging Sapi Teriyaki & Cah Jagung Putren",
        "Kategori": "Lauk Utama",
      ...
      },
      {
        "Nama Menu": "Daging Sapi Teriyaki & Cah Jagung Putren",
        "Kategori": "Lauk Utama",
        "Bahan Baku": "Jagung Putren (Muda)",
        "Satuan": "Gram",
        "Porsi Besar (SD/SMP)": 50,
        "Porsi Kecil (TK/PAUD)": 30,
        "Kalori (Kkal)": 450,
        "Protein (g)": 30,
        "Karbohidrat (g)": 45,
        "Lemak (g)": 15
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Template Menu BOM");
    ...
    const wsHelp = XLSX.utils.json_to_sheet([
        ...
        { Catatan: "3. Kolom yang mengandung kata 'Porsi' (misal: Porsi Besar) dianggap sebagai Jenis Porsi." },
        { Catatan: "4. Anda dapat menambah kolom 'Porsi Lainnya' ke samping jika ada lebih dari 2 jenis porsi." },
    ]);
    ...
    try {
        ...
        link.href = url;
        link.download = "Template_Master_Menu_BOM.xlsx";
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const res = await menuApi.createBulk({ menus: menusArray });
        alert(`Berhasil mengunggah ${res.count} Menu beserta detail BOM dan gizinya!`);
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={downloadTemplate} className="flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-700 rounded-lg text-sm font-semibold hover:bg-slate-50 transition-all shadow-sm">
             <FiDownload className="text-slate-400" /> Unduh Template Excel
          </button>
          ...
          <button disabled={uploading} onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 transition-all shadow-sm disabled:opacity-50">
             <FiUpload /> {uploading ? "Memproses..." : "Upload BOM Massal"}
          </button>
          <div className="w-px h-6 bg-gray-200 hidden sm:block"></div>
          <button onClick={openCreateMenu} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-semibold hover:bg-slate-800 transition-all shadow-sm">
            <FiPlus /> Buat Manual
          </button>
        </div>
      </div>
      ...
      {portionTypes.length === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
          ⚠️ Anda belum membuat setidaknya satu jenis porsi. <a href="/dashboard/admin-pusat/porsi" className="underline font-medium">Buat Jenis Porsi terlebih dahulu</a>
        </div>
      )}
      ...
      {menus.length === 0 ? (
        <div className="bg-white rounded-xl border-2 border-dashed border-gray-200 p-12 text-center">
          ...
          <h3 className="font-semibold text-gray-500">Belum Ada Master Menu</h3>
          <p className="text-sm text-gray-400 mt-1 mb-6">Mulai standarisasi resep dengan membuat master menu pertama Anda atau Upload Excel.</p>
          <button onClick={() => fileInputRef.current?.click()} className="px-6 py-2.5 bg-slate-900 text-white rounded-lg text-sm font-bold hover:bg-slate-800 inline-flex items-center gap-2 shadow-sm"><FiUpload /> Upload File Excel</button>
        </div>
      ) : (
        <div className="space-y-3">
          {menus.map(menu => (
            <div key={menu.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              {/* Header */}
                  ...
                  <div className="p-5">
                      ...
                      {editingIngredients !== menu.id ? (
                        ...
                        menu.ingredients.length === 0 ? (
                          <p className="text-sm text-gray-500 text-center py-8 bg-white/50 rounded-xl border border-gray-200 border-dashed">Belum ada bahan baku dikonfigurasi. Klik "Konfigurasi Bahan" untuk menyetel standarisasi resep BOM.</p>
                        ) : (
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-slate-50/50">
              <h2 className="text-xl font-bold text-slate-800">{editingMenu ? "Edit Master Menu" : "Buat Master Menu Baru"}</h2>

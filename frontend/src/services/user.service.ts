...
class UserService {
  ...
  async getAllUsers(filters?: UserFilters): Promise<UserListResponse> {
    ...
    // Role scoped by backend: SUPER_ADMIN, PROJECT_OWNER, ADMIN_PUSAT, ADMIN_DAPUR and team roles use /users
    const endpoint = (
      userRole === 'SUPER_ADMIN' ||
      userRole === 'PROJECT_OWNER' ||
      userRole === 'ADMIN_PUSAT' ||
      userRole === 'ADMIN_DAPUR' ||
      userRole === 'AKUNTAN' ||
      userRole === 'PRODUKSI' ||
      userRole === 'AHLI_GIZI' ||
      userRole === 'CHEF' ||
  ...
  async getSuppliers(): Promise<User[]> {
    const headers = await this.getAuthHeaders();
    const response = await fetch(`${API_BASE_URL}/users?role=SUPPLIER`, {
      method: 'GET',
      headers,
    });
    const result = await this.handleResponse<UserListResponse>(response);
